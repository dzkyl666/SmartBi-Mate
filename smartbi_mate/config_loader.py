...
from smartbi_mate.constants import SQL_RESULT_LIMIT
...
class Config(BaseModel):
    """Configuration model for the smartbi_mate application.
...
class ConfigLoader:
    """Singleton class to load and manage configuration settings for smartbi_mate.
    ...
    def load(self, config_file: str | None = None) -> None:
        ...
        Args:
            config_file (str, optional): Path to configuration file. Uses CONFIG_FILE
                environment variable or 'smartbi_mate/config.yaml' if not provided.
        ...
        if config_file is None:
            config_file = os.getenv("CONFIG_FILE", "smartbi_mate/config.yaml")
    ...
    def _instantiate_from_config_dict(self, config_item: dict[str, Any], *, config_key: str) -> Any:
        try:
            ...
            params = dict(config_item.get("params", {}))

            # 为 OpenAI 兼容类注入 trust_env=False，避免国内 API 走系统代理超时
            # 注意：只对 ChatOpenAI 注入 http_client；OpenAIEmbeddings 不注入，
            # 因为 langchain-openai 1.6.x 的 tokenize 逻辑与自定义 http_client 冲突，
            # 会导致百炼 API 收到 token IDs 而非字符串报错
            if "ChatOpenAI" in class_path:
                import httpx
                params.setdefault("http_client", httpx.Client(trust_env=False))

            # api_key 从环境变量读取（不硬编码在 YAML 中）
            if "api_key" not in params and "openai" in class_path.lower():
                import os
                env_key = os.environ.get("DASHSCOPE_API_KEY", "")
                if not env_key:
                    # 子进程可能没继承 User 级环境变量，尝试直接读注册表
                    try:
                        import winreg
                        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Environment")
                        env_key, _ = winreg.QueryValueEx(key, "DASHSCOPE_API_KEY")
                        winreg.CloseKey(key)
                    except Exception:
                        pass
                if env_key:
                    ...
                    log("WARN: DASHSCOPE_API_KEY not set, LLM calls will fail")

            # ChatOpenAI 需要 http_async_client 来避免 async 初始化报错
            if "ChatOpenAI" in class_path and "http_async_client" not in params:
                import httpx
                params.setdefault("http_async_client", httpx.AsyncClient(trust_env=False))
