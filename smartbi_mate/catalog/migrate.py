...
one :class:`~smartbi_mate.catalog.catalog_store.CatalogStore` to another. The most
...
CLI usage::
    ...
    python -m smartbi_mate.catalog.migrate \
        --source-path ./example \
...
from smartbi_mate.catalog.catalog_store import CatalogStore, split_db_table_name
...
def _build_source(args: argparse.Namespace) -> CatalogStore:
    from smartbi_mate.catalog.store.file_system import FileSystemCatalogStore

    return FileSystemCatalogStore(data_path=args.source_path)


def _build_dest(args: argparse.Namespace) -> CatalogStore:
    from smartbi_mate.catalog.store.database import DatabaseCatalogStore
