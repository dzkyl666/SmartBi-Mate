...
from smartbi_mate.catalog import CatalogStore
from smartbi_mate.catalog.schema_retrival import col_dict, column_tables_mapping, get_relevant_columns
from smartbi_mate.constants import datetime_format
from smartbi_mate.graph_state import SQLGraphState
from smartbi_mate.prompts.system_prompt import get_table_selection_prompt_template
from smartbi_mate.text2sql.data import table_selection_example_dict, table_selection_retriever
from smartbi_mate.utils import extract_json_from_answer, get_text_from_content, log
...
def schema_linking(llm: BaseChatModel, catalog: CatalogStore):
    ...
    def _get_related_tables_and_columns(keywords_list, dimensions, metrics, start_time=None, invalid_table=None):
        ...
        # 如果通过列映射找不到候选表，直接回退到 catalog 中所有已注册的表
        if not candidate_tables:
            log("No candidate tables from column mapping, falling back to all registered tables")
            try:
                all_tables = catalog.get_table_list()
                candidate_tables = set(all_tables)
            except Exception as e:
                log(f"Failed to get table list: {e}")
                candidate_tables = set()
            log(f"Fallback candidate tables: {candidate_tables}")

        # 最终兜底：如果仍然为空，强制加入核心业务表
        if not candidate_tables:
            log("WARN: Still no candidate tables after fallback, forcing core tables")
            candidate_tables = {"fact_order", "dim_region", "dim_product", "dim_user"}
            log(f"Forced candidate tables: {candidate_tables}")
        if start_time:
            try:
    ...
    def _call_llm_select(llm: BaseChatModel, system_prompt, messages, question, candidate_tables) -> dict[str, Any]:
        ...
        candidate_list = ", ".join(candidate_tables) if candidate_tables else "(none)"
        log(f"Candidate tables: {candidate_list}")
        prompt = f"""请从以下候选表中选择合适的表来回答问题：{question}

⚠️ 重要：你只能选择下面列出的候选表，不要编造不存在的表名。
候选表列表：[{candidate_list}]

如果候选表为空，说明没有匹配的表，请返回空的 tables 数组。"""
        ...
        while retry_flag:
            try:
                ...
                if _verify_table(selected_tables, candidate_tables):
                    ...
                    bad_tables = ",".join([table.get("table") for table in result.get("tables") or []])
                    messages.append(
                        HumanMessage(
                            f'你选择的表 [{bad_tables}] 不在候选表列表中。'
                            f'候选表只有：[{candidate_list}]。'
                            f'请只从候选表中选择，不要编造表名。'
                        )
                    )
                retry_cnt += 1
