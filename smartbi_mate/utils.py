"""Utility functions for smartbi_mate."""
...
from smartbi_mate.graph_state import AgentState
from smartbi_mate.text_segmenter import _segmenter
...
class _DashScopeEmbeddingWrapper:
    """百炼 Embedding API 包装器。

    langchain-openai 1.6.x 的 OpenAIEmbeddings 在 tokenize 后会把字符串变成
    token ID 列表再发给 API，但百炼只接受原始字符串。这个包装器绕过 tokenize，
    直接用 openai SDK 发送字符串。
    """
    ...
    def embed_documents(self, texts: list[str]) -> list[list[float]]:
        if not texts:
            return []
        # 百炼 text-embedding-v4 单次最多 10 条，分批请求
        # 端点间歇性返回 400 InternalError（实测 ~25% 概率），加重试
    ...
    def _embed_with_retry(self, batch: list[str], max_attempts: int = 3):
        """带重试的 embedding 请求，降低间歇性 400 InternalError 的影响。"""
        last_err = None
        ...
        raise last_err  # 重试耗尽，上抛给调用方（column_retrieval 的 try/except 会再兜一次）
...
def _create_chroma_from_texts(
    ...
    # 如果 embedding 是 OpenAIEmbeddings 且 base_url 指向百炼，
    # 替换为不走 tokenize 的包装器
    actual_embedding = embedding
    try:
        from langchain_openai import OpenAIEmbeddings
        if isinstance(embedding, OpenAIEmbeddings):
            base_url = getattr(embedding, "openai_api_base", None) or ""
            if "dashscope" in base_url:
                import os, winreg
                # 优先从 config_loader 已注入的 params 中获取（最可靠）
                api_key = getattr(embedding, "openai_api_key", None) or ""
                # openai_api_key 可能是 SecretStr 对象
                ...
                if not api_key:
                    try:
                        ...
                        pass
                model = getattr(embedding, "model", "text-embedding-v4")
                actual_embedding = _DashScopeEmbeddingWrapper(api_key, base_url, model)
                log(f"Using DashScope embedding wrapper for {model}")
    except Exception as e:
        log(f"DashScope embedding wrapper check failed: {e}, using original embedding")
...
def create_vector_db(
    ...
    """
    # 如果没有配置 embedding 模型，回退到 BM25 简单向量存储
    if not embedding:
    ...
    if not texts:
        log("WARN: No valid texts to embed, falling back to SimpleStore")
        return SimpleStore([], metadatas)

    # 如果 embedding 是 OpenAIEmbeddings 且指向百炼，替换为不走 tokenize 的包装器
    actual_embedding = embedding
    try:
        from langchain_openai import OpenAIEmbeddings
        if isinstance(embedding, OpenAIEmbeddings):
            base_url = getattr(embedding, "openai_api_base", None) or ""
            if "dashscope" in base_url:
                import os as _os, winreg as _wr
                api_key = getattr(embedding, "openai_api_key", None) or ""
                if hasattr(api_key, "get_secret_value"):
                    api_key = api_key.get_secret_value()
                if not api_key:
                ...
                if not api_key:
                    try:
                        _k = _wr.OpenKey(_wr.HKEY_CURRENT_USER, r"Environment")
                        api_key, _ = _wr.QueryValueEx(_k, "DASHSCOPE_API_KEY")
                        _wr.CloseKey(_k)
                    except Exception:
                        pass
                model = getattr(embedding, "model", "text-embedding-v4")
                actual_embedding = _DashScopeEmbeddingWrapper(api_key, base_url, model)
                log(f"Using DashScope embedding wrapper for {model} (columns)")
    except Exception as e:
        log(f"DashScope embedding wrapper check failed: {e}")

    chroma_dir = chroma_db_path or "./.chroma_db"
    client = Chroma(
        collection_name,
        persist_directory=chroma_dir,
        embedding_function=actual_embedding,
        collection_metadata=collection_metadata,
    )
    use_cache = False
    existing_docs = None
